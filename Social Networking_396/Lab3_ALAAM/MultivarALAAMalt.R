...
prepALAAMdata <- function(y,ADJ,covariates,directed=FALSE,useDegree = FALSE, contagion ='simple', interaction=NULL)
{
	n <- dim(ADJ)[1]
	RowIn <- matrix(NA,n,1)
	degree <- matrix(0,n,1)
...
plotPost <- function(ALAAMresult,figname,nameaxs=NULL,sizearg=NULL,doTable=FALSE,thinning=NULL,burnin = NULL)
{
	require('xtable')
	require('coda')
	Iterations <- dim(ALAAMresult$Thetas)[1]
...
pathsampALAAM <- function(ALAAMobj,theta_tilde,theta_star,burnin=3000,numbridges=20,interaction=interaction,sampleSize=100,canchange=NULL,silent=FALSE)
{
...
translist <- matrix(c(999,0,0),1,3)
First <- vector('list',n)
...
translist <- translist[translist[,1]<999,]
...
write.res.table <- function(burnin=1,datamat=NULL,thin=1,tabname=NULL,nameVec=NULL)
{
...
get.gof.distribution <- function(NumIterations=NULL,res=NULL,burnin=100,thinning= 1000, contagion ='simple')
{
  ...
  start_time <- Sys.time()
  Sav.gof.2 <- simulate.alaam.NULL(ALAAMobj=mod.1,contagion =c('simple','recip','indirect','closedind','transitive'), DoSave=TRUE, returnNet= TRUE, doGOF=TRUE, sim.data= Big.stats$y)
  
  Sav.gof <- Sav.gof.2$statsvec
  end_time <- Sys.time()
  cat('Calculating statistics took in second way ',end_time - start_time,'\n')
 ...
 # pairs(~Sav.gof[3,]+Sav.gof[4,]+Sav.gof.2$statsvec[3,]+Sav.gof.2$statsvec[4,],
  ...
  gof.stats.names<- c('intercept' ,'simple cont.','recip cont.','indirect cont.','closedind cont.','transitive cont.','indegree','outdegree','twopaths','out2star','in2star','outtria','intria','transtri','indirecct ties')
  gof.out <- list(Sav.gof=Sav.gof,ADJ=ADJ,stats=stats, y = res$ALAAMobj$y, gof.stats.names=gof.stats.names, Imp.gof=Imp.gof)
