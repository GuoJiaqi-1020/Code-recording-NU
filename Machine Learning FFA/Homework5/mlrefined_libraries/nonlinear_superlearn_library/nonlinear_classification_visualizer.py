...
class Visualizer:
    ...
    def plot_data(self):
        ...
        if np.shape(self.x)[1] < 2:
            ...
            ### plot in 1-d
            ind0 = np.argwhere(self.y == +1)
            ax2.scatter(self.x[ind0], np.zeros((len(self.x[ind0]))), s=55, color=self.colors[1], edgecolor='k',
                        zorder=3)

            ind1 = np.argwhere(self.y == -1)
            ax2.scatter(self.x[ind1], np.zeros((len(self.x[ind1]))), s=55, color=self.colors[0], edgecolor='k',
                        zorder=3)
            ax2.set_yticks([0])
    ...
    def show_individual_classifiers(self, run, w, **kwargs):
        ...
        for d in range(C):
            ...
            ax.contour(w1_vals, w2_vals, g_2 + 1, colors=self.colors[d], levels=[-1, 1], linewidths=2.5, zorder=1,
                       linestyle='-')
