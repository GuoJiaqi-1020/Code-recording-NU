...
def gradient_descent(g, counter, x_train, y_train, x_valid, y_valid, alpha, max_its, w, num_pts, batch_size, verbose,
                     version):
    if verbose:
    ...
    for k in range(max_its):
        ...
        if verbose:
            print('step ' + str(k + 1) + ' done in ' + str(np.round(end - start, 1)) + ' secs, train acc = ' + str(
                np.round(train_count_hist[-1], 4)) + ', valid acc = ' + str(np.round(valid_count_hist[-1], 4)))
