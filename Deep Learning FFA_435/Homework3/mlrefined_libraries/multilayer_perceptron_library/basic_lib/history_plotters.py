...
class Setup:
    def __init__(self, train_cost_histories, train_accuracy_histories, val_cost_histories, val_accuracy_histories,
                 start):
        # plotting colors
        ...
        if len(train_accuracy_histories) == 0:
            ...
            self.plot_cost_count_histories(train_cost_histories, train_accuracy_histories, val_cost_histories,
                                           val_accuracy_histories, start)
    ...
    def plot_cost_histories(self, train_cost_histories, val_cost_histories, start):
        ...
        for c in range(len(train_cost_histories)):
            ...
            ax.plot(np.arange(start, len(train_history), 1), train_history[start:], linewidth=3 * (0.8) ** (c),
                    color=self.colors[0], label='train cost')

            if np.size(val_history) > 0:
                # plot test cost function history
                ax.plot(np.arange(start, len(val_history), 1), val_history[start:], linewidth=3 * (0.8) ** (c),
                        color=self.colors[1], label='validation cost')
    ...
    def plot_cost_count_histories(self, train_cost_histories, train_accuracy_histories, val_cost_histories,
                                  val_accuracy_histories, start):
        # initialize figure
        fig = plt.figure(figsize=(10, 3))
        ...
        for c in range(len(train_cost_histories)):
            ...
            ax1.plot(np.arange(start, len(train_cost_history), 1), train_cost_history[start:],
                     linewidth=3 * (0.8) ** (c), color=self.colors[0])

            ax2.plot(np.arange(start, len(train_accuracy_history), 1), train_accuracy_history[start:],
                     linewidth=3 * (0.8) ** (c), color=self.colors[0], label='train')
